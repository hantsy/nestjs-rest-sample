...
- _src/app.*_ is the top-level component in a Nest application.
  - There is an *app.module.ts*, which is a Nestjs `Module`, similar to Angular `NgModule`, and used to organize code in the logic view.
  - The _app.service.ts_ is an `@Injectable` component, similar to the service in [Angular](https://spring.io) or Spring's Service, it is used for handling business logic. A service is annotated with `@Injectable`.
  - The _app.controller.ts_ is the controller of MVC, to handle incoming requests, and respond with the handled result back to the client. The annotation `@Controller()` is similar to Spring MVC's `@Controller`.
  - The _app.controller.spec.ts_ is test file for _app.controller.ts_. NestJS uses [Jest](https://jestjs.io/) as the testing framework.
...
- PUT /posts/id - Update a certain post specified by id
...
- A `PostService` component to serve the data for the controller. In this post, we use dummy data for storage temporarily; we will replace it with a real MongoDB in a future post.
...
`PosModule` is imported into the top-level `AppModule`. Check the content of the file _src/app/app.module.ts_:
...
The `PostService` is added to the `PostModule` automatically when it is generated.
...
After it is done there are 4 files created in the _src/app/post_ folder, including a _spec_ file for testing `PostService`.
...
- In the `constructor`, we add `PostService` as its argument; the `PostService` component will be injected automatically.
- The `Controller` annotation indicates it is a controller, and it uses `/posts` as the base URI for all methods.
- The `Get`, `Post`, `Put`, and `Delete` methods are used for handling different HTTP methods.
- The `@Param` binds the path parameters (`:id`) to the method arguments, and there is `ParseIntPipe` applied in the second argument of Param. Pipe is an Angular concept, here it is closer to Spring's converters and validators.
- `@Body` parses the request body into a `Post` object.

> In the Typescript language, we call the `@Controller` syntax as `Decorator`. For those familiar with Spring WebMVC, the word **annotation** is easier to understand.
...
In the following post, we will connect to a real database to replace the dummy data service.
